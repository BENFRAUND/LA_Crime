...
d3.json("/crime_sites", function (data) {
  ...
  data.forEach(function (sqlitedata) {
    ...
    crimeSites.push(
      ...
      ).bindPopup(data.crm_cd_desc.bold() + "<br>" + data.fbi_cat.bold() + " (actual category)<br>" + data.predict_cat.bold() +
       " (predicted category)<br>" + titleCase(data.location) + "<br>" + data.area_name + " Area<br>" + data.date_occ.toString().slice(0,16) +
        " @ " + data.hour_occ + ":00<br>DR#: " + data.dr_no.toString().bold()))
  });
