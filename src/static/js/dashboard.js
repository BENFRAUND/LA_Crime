  ...
  d3.json("/crime_sites/"+dr_no).then(function(metaData) {
    ...
    var site_id = metaData.dr_no;
      ...
      d3.select("#sample-metadata")
        .html(`<h5>Police reported a ${metaData.crm_cd_desc.toLowerCase()} occurred at ${titleCase(metaData.location)} in the ${metaData.area_name}
                area on ${metaData.date_occ.substring(0,16)}.  The following modus operandi were described in the police report: ${metaData.mocodes.replace('["','<br><ul><li>')
                                                                                                                  .replace('", "','<li>')
                                                                                                                  .replace('", "','<li>')
                                                                                                                  .replace('", "','<li>')
                                                                                                                  .replace('", "','<li>')
                ...
                This crime is reportable to the FBI as a ${titleCase(metaData.FBI_Category)}. The deep neural net model predicted this
                 as a ${titleCase(metaData.FBI_Cat_Prediction)} based on evidence in the police report other than the crime code or weapon.</h5>
                `);
